...
const upload = multer({ storage: multer.memoryStorage() }); // ✅ fixed typo here

app.post('/merge', upload.fields([{ name: 'audio' }, { name: 'video' }]), (req, res) => {
  const audioBuffer = req.files['audio'][0].buffer;
  const videoBuffer = req.files['video'][0].buffer;

  if (!fs.existsSync('inputs')) fs.mkdirSync('inputs');     // ✅ create inputs directory
  if (!fs.existsSync('outputs')) fs.mkdirSync('outputs');   // ✅ create outputs directory
